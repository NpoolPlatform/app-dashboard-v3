...
const onSubmit = (done: () => void) => {
  if (updating.value) {
    sdk.updateTopMostPoster(target.value, (error: boolean) => {
      done()
      if (error) return
      onMenuHide()
    })
  } else {
    sdk.createTopMostPoster(target.value, (error: boolean) => {
      done()
      if (error) return
      onMenuHide()
    })
