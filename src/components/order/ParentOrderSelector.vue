...
const _orders = computed(() => Array.from(orders.value).map((el) => {
  return {
    value: el.ParentOrderID,
    label: `${el.EntID} | ${el.ParentOrderID} | ${el.OrderType} | ${el.AppGoodID} | ${el.AppGoodName} | ${el.GoodType}`
  }
}))
const displayOrders = ref(_orders.value)

const emit = defineEmits<{(e: 'update:parentOrderId', parentOrderId: string): void}>()
const onUpdate = () => {
  emit('update:parentOrderId', target.value)
}
